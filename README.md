...
|--------------|----------|--------------------------------------------|-------------|
| MasterHonda  | ESP32    | Master — relay/button input, ESP-NOW hub  | Pin Status, Nodes, OTA Update |
...
- **SlaveHonda WiFi-optional fallback** — if it can't reach the router, it scans ESP-NOW channels for MasterHonda directly and keeps operating without an IP
- **Native captive portal** — first-startup WiFi setup with network scan, served by `esp_http_server` directly
- **Web UI per unit** — live pin status, node roster with per-link RSSI/channel (MasterHonda), manual start/stop override, OTA update
...
MasterHonda and SlaveWallas require WiFi. SlaveHonda prefers it but can operate WiFi-less over ESP-NOW.
